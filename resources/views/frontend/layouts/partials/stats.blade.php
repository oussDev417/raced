...
<section class="ul-counter-section ul-section-spacing">
    <div class="ul-counter-overlay"></div>
    <div class="ul-container">
        <div class="ul-section-heading text-center wow animate__fadeInUp mb-5">
            <span class="ul-section-sub-title text-white">Nos chiffres</span>
            <h2 class="ul-section-title text-white">Notre impact en chiffres</h2>
        </div>
        <img src="assets/img/about_shape2_1.png" alt="Vector" class="ul-cta-vector">
        <div class="ul-counter-wrapper">
            <div class="ul-counter-item wow animate__fadeInUp" data-wow-delay="0.1s">
                <div class="ul-counter-icon">
                    <i class="flaticon-team"></i>
                </div>
                <div class="ul-counter-content">
                    <h2 class="counter-number">
                        <span class="counter">{{ $statFacts[0]->counter ?? '5468' }}</span>
                        <span class="suffix">+</span>
                    </h2>
                    <p class="counter-title">{{ $statFacts[0]->title ?? 'Donateurs' }}</p>
                </div>
            </div>

            <div class="ul-counter-item wow animate__fadeInUp" data-wow-delay="0.3s">
                <div class="ul-counter-icon">
                    <i class="flaticon-checked"></i>
                </div>
                <div class="ul-counter-content">
                    <h2 class="counter-number">
                        <span class="counter">{{ $statFacts[1]->counter ?? '6875' }}</span>
                        <span class="suffix">+</span>
                    </h2>
                    <p class="counter-title">{{ $statFacts[1]->title ?? 'Projets réalisés' }}</p>
                </div>
            </div>

            <div class="ul-counter-item wow animate__fadeInUp" data-wow-delay="0.5s">
                <div class="ul-counter-icon">
                    <i class="flaticon-relationship"></i>
                </div>
                <div class="ul-counter-content">
                    <h2 class="counter-number">
                        <span class="counter">{{ $statFacts[2]->counter ?? '6875' }}</span>
                        <span class="suffix">+</span>
                    </h2>
                    <p class="counter-title">{{ $statFacts[2]->title ?? 'Bénévoles' }}</p>
                </div>
            </div>

            <div class="ul-counter-item wow animate__fadeInUp" data-wow-delay="0.7s">
                <div class="ul-counter-icon">
                    <i class="flaticon-cart"></i>
                </div>
                <div class="ul-counter-content">
                    <h2 class="counter-number">
                        <span class="counter">{{ $statFacts[3]->counter ?? '45000' }}</span>
                        <span class="suffix">€</span>
                    </h2>
                    <p class="counter-title">{{ $statFacts[3]->title ?? 'Montant collecté' }}</p>
                </div>
            </div>
        </div>
    </div>
...
.ul-counter-section {
	position: relative;
	background: url(https://images.unsplash.com/photo-1509099836639-18ba1795216d?q=80&w=1931&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D) no-repeat center center/cover;
	z-index: 1;
	text-align: center;
	padding: clamp(60px, 10.51vw, 200px) 0;
}
.ul-counter-section::after {
	content: "";
	position: absolute;
	inset: 0;
